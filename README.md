...
`dormant.enable=true` のみでポートを持たない補助コンテナ（例: 起動トリガー専用の `awaken` など）は、dormant が起動した場合、**参照元の子が全て停止したら自動で停止**されます。参照元の子が1つでもアクティブ（アクセスがあり `dormant.session-duration` 以内、またはアクティブ接続中）なら停止されません。

- 参照元の子が複数いても、全部停止すれば補助コンテナも回収されます
- 手動で起動したコンテナは停止対象になりません（dormant が起動したもののみ）
- バックグラウンド処理のみで HTTP アクセスが来ない子は、`dormant.session-duration` を十分に設定してください（最終アクセスから一定時間で処理が終わる前提）
